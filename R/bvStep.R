...
#' The test is the same as that performed by the \code{\link[mantelNSGA2]{bioEnv}} function but the routine provides a more efficient
...
#' (see \code{\link[mantelNSGA2]{bioEnv}} for an explanation to these types of setups). Defaults to \code{max.rho=0.95}
...
#' (see \code{\link[mantelNSGA2]{bioEnv}} for an explanation of other types of analyses
#' beyond the typical "BIOENV").
#' Below is an example of a two-step search refinement for searching
#' for subsets of variables that best correlate with a fixed multivariate set.
#'
#' @references
